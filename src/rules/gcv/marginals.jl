...
@marginalrule GCV(:y_x) (m_y::Any, m_x::Any, q_z_κ::Any, q_ω::Any) = begin
    ...
    # A = expectation( x -> exp(-x[1]*x[2]), q_z_κ)
    B = expectation( x -> exp(-x) , Normal(mean(q_ω),var(q_ω)))
...
@marginalrule GCV(:y_x) (m_y::Any, m_x::Any, q_z_κ_ω::Any,meta::GCVMetadata) = begin
...
@marginalrule GCV(:z_κ) (q_y_x::Any, m_z::Any,m_κ::Any, q_ω::Any,meta::GCVMetadata) = begin
    ...
    B = expectation( x -> exp(-x) , Normal(mean(q_ω),std(q_ω)))
...
@marginalrule GCV(:y_x_z) (m_y::Any,m_x::Any, m_z::Any,q_κ::Any, q_ω::Any,meta::GCVMetadata) = begin
    ...
    f = x -> exp(-0.5*(mean(q_κ)*x[3]+x[3] + A*h(x[3])+ (x[2]-x[1])^2)+logpdf(m_y,x[1])+logpdf(m_x,x[2])+logpdf(m_z,x[3]))
